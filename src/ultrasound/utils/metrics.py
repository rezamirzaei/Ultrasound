...
def compute_confusion_matrix(
    ...
    return cast(
        np.ndarray,
        sklearn_confusion_matrix(target.flatten(), pred.flatten(), normalize=normalize),
    )
...
def compute_classification_metrics(
    ...
    """
    precision, recall, f1, support = precision_recall_fscore_support(target, pred, average=None)
...
def compute_psnr(
    pred: np.ndarray, target: np.ndarray, *, data_range: Optional[float] = None
) -> float:
...
def compute_ssim(
    ...
    Notes:
        Uses skimage if available; returns NaN if skimage is missing.
    """
    try:
    ...
    # skimage expects 2D for images; for 1D signals SSIM is less standard.
    if pred.ndim == 1:
        pred = pred[None, :]
        target = target[None, :]
