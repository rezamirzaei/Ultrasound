...
class DownBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return cast(torch.Tensor, self.maxpool_conv(x))
...
class AttentionGate(nn.Module):
    ...
    def forward(self, g: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        return cast(torch.Tensor, x * psi)
...
class AttentionUNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return cast(torch.Tensor, self.outc(d1))
...
def dice_loss(
    ...
    Args:
        pred: Predicted logits (if from_logits=True) or probabilities
        target: Ground truth binary mask
        smooth: Smoothing factor to avoid division by zero
        from_logits: If True, apply sigmoid to pred first. Set to False
            when pred already contains probabilities.
    ...
    return cast(torch.Tensor, 1 - dice)
...
def combined_loss(
    ...
    Both components receive raw logits and apply sigmoid internally,
    so ``pred`` must be logits (not probabilities).
    ...
    return cast(torch.Tensor, bce_weight * bce + (1 - bce_weight) * dice)
