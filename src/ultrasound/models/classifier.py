...
def focal_loss(
    ...
    Supports multi-class targets via cross-entropy. The ``alpha`` parameter
    is applied uniformly to all classes; for per-class weighting, use
    ``torch.nn.CrossEntropyLoss(weight=...)``.

    Args:
        pred: Predicted logits of shape (N, C)
        target: Ground truth class indices of shape (N,)
        alpha: Uniform weighting factor applied to all samples (set None to disable)
...
class EnsembleClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        logits = [cast(torch.Tensor, model(x)) for model in self.models]
